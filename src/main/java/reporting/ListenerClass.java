...
public class ListenerClass extends BaseClass implements ITestListener,IInvokedMethodListener{
	...
	public void onTestFailure(ITestResult result) {
		...
		AndroidDriver androiddriver = ((BaseClass) currentClass).getAndroidDriver();
		try
		{
			...
			if(((BaseClass) currentClass).getClientName().equals("CAP"))
				ReportBaseClass.takeSnapShot(driver,file);
			if(((BaseClass) currentClass).getClientName().equals("MAP"))
				ReportBaseClass.takeSnapShotMAP(androiddriver,file);
			if(((BaseClass) currentClass).getClientName().equals("E2E"))
			{
				ReportBaseClass.takeSnapShotMAP(androiddriver,file);
				ReportBaseClass.takeSnapShot(driver,file);
			}
			...
			if(((BaseClass) currentClass).getClientName().equals("CAP"))
				...
				Reporter.log("NA");
			else if(((BaseClass) currentClass).getClientName().equals("MAP"))
				Reporter.log(file);
			else if(((BaseClass) currentClass).getClientName().equals("E2E"))
				Reporter.log(file);
